...
9. **Run LOR baseline and comparison** (~30s, no LLM calls):
   ```bash
   RESULTS_DIR="outputs/blis_router/<YYYYMMDD_HHMMSS>"
   python benchmarks/blis_router/scripts/run_lor_baseline.py --output-dir "$RESULTS_DIR/lor_baseline"
   python benchmarks/blis_router/scripts/compare_lor.py --lor "$RESULTS_DIR/lor_baseline/lor_metrics.json" --experiment "$RESULTS_DIR"
   ```
   Produces `lor_metrics.json` and `lor_comparison.json`. Shows 3-way comparison: LOR (pure load-balance) → Baseline (prefix-affinity + LB) → Discovered. Reveals whether evolved algorithms beat simple load-balancing or just undo harmful prefix-affinity.
