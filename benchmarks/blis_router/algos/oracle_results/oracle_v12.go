...
func (ws *WeightedScoring) Route(req *Request, state *RouterState) RoutingDecision {
	...
	// EVOLVE-BLOCK-START
	// Oracle v12: Prefix-Spread Router (Low Prefix Weight)
	//
	// KEY INSIGHT: With binary prefix scores (1.0 vs 0.0), the 1:1 baseline's
	// 50/50 prefix/load split ALWAYS routes to the cached instance because
	// 0.5*1.0 + 0.5*load > 0.5*0.0 + 0.5*1.0 for any positive load score.
	//
	// By using a LOWER prefix weight (30%), we allow the load signal to occasionally
	// override prefix affinity. This causes natural overflow:
	//   cached:   0.3*1.0 + 0.7/(1+InFlight_cached)
	//   uncached: 0.3*0.0 + 0.7/(1+InFlight_uncached)
	//
	// When cached instance has InFlight >= 1 and uncached has 0:
	//   cached = 0.3 + 0.7/2 = 0.65, uncached = 0.7 → uncached wins
	//
	// This overflow has a CRUCIAL side effect: the prefix cache observer records
	// block hashes on the overflow instance. After a few overflows, the second
	// instance also has the dominant group's prefix blocks cached. Now BOTH
	// instances get prefix score 1.0 for that group, and load-balancing
	// distributes traffic naturally between them.
	//
	// Result: dominant groups spread across 2+ instances (cache hits + load balance)
	// vs 1:1 which locks them to 1 instance (cache hits but hotspot)
	// vs LOR which spreads evenly (load balance but no cache hits)

	prefixScores := ws.scorers[0](req, snapshots)
	scores := make(map[string]float64, len(snapshots))

	// Low prefix weight allows load to override during bursts
	prefixWeight := 0.30
