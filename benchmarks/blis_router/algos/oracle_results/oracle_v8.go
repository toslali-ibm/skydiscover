...
func (ws *WeightedScoring) Route(req *Request, state *RouterState) RoutingDecision {
	...
	// EVOLVE-BLOCK-START
	// Oracle v8: Fresh-Signal with Quadratic Load + Hybrid Load Metric
	//
	// Key insight from v4: using InFlightRequests (fresh) instead of EffectiveLoad (stale)
	// gives +1.21% improvement. This version tries to squeeze more by:
	// 1. Using 1/(1+InFlightRequests)^2 for stronger load differentiation
	// 2. Adding a small EffectiveLoad tiebreaker for when InFlightRequests ties
	// 3. Small KV headroom bonus for very high utilization
	//
	// We keep 50/50 prefix/load split since higher prefix doesn't change routing
	// decisions (prefix is binary: 1.0 or 0.0 for prefix_heavy).
	...
	for _, snap := range snapshots {
		...
		// Quadratic decay: stronger preference for less loaded instances
		// 1/(1+0)^2 = 1.0, 1/(1+1)^2 = 0.25, 1/(1+2)^2 = 0.11
		// vs linear: 1/(1+0) = 1.0, 1/(1+1) = 0.5, 1/(1+2) = 0.33
		freshLoad := 1.0 / ((1.0 + inFlight) * (1.0 + inFlight))

		// Small stale load tiebreaker (when InFlightRequests ties)
		staleTiebreak := 0.001 / (1.0 + float64(snap.QueueDepth+snap.BatchSize))

		// KV saturation avoidance
		kvBonus := 0.0
		if snap.KVUtilization > 0.9 {
			kvBonus = -0.01 // small penalty
		}

		scores[snap.ID] = 0.5*ps + 0.5*(freshLoad+staleTiebreak) + kvBonus
	}
